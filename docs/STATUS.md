...
**Последнее обновление**: 2026-09-11 (этап 17 взят в работу — веб-панель управления)
**Текущий этап**: 17 — веб-панель управления `/admin`. Этапы 0–16 закрыты.
**Следующий шаг**: собрать панель, отдать клиенту логин и пароль, после приёмки вывести бота.
...
**Этап 17 — веб-панель управления `/admin`.** Взят в работу 2026-09-11. Клиент не может
пользоваться ботом в Telegram, поэтому управление сайтом переезжает в браузер: вход по
логину и паролю, адаптив под телефон, всё, что умел бот, плюс баннеры и список песен.
Бот удаляется из проекта **только после приёмки панели клиентом** — до этого он остаётся
единственным рабочим способом публиковать контент.
