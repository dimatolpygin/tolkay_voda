...
**Последнее обновление**: 2026-06-17 (открыт майлстоун 2 — пост-MVP правки, этапы 10–12)
**Текущий этап**: майлстоун 1 (0–9) закрыт и принят клиентом. Открыт **майлстоун 2** (этапы 10–12): редактирование контента через бота, живой эфир (Icecast+Liquidsoap), SEO под капотом. План в [`07_ROADMAP.md`](07_ROADMAP.md).
**Следующий шаг**: взять в работу этап 10 (редактирование контента через Telegram-бота).
...
| 10 | Редактирование контента (бот) | 🚧 | `stage-10-done` | — | — |
...
**Этап 10 — Редактирование контента через бота** 🚧 — **код готов, ждёт живой проверки клиентом** (тег не ставим до UAT).
Реализовано: вход «Редактировать» (`/start` + `/edit`) → выбор типа → список последних 10 записей →
меню полей (заголовок/текст/фото/структурные поля прогноза/удалить) → ввод значения/фото/подтверждение.
Доступ — whitelist. store-функции прогнаны на временной БД (CRUD + whitelist полей).
Верификация: клиент в боте правит/удаляет реальную запись, смотрим изменение на сайте.
