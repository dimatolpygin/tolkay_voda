...
**Следующий шаг**: A-запись `tolkay-voda.ru` → `89.125.121.199` (за пользователем).
Новый сервер полностью готов: 5 контейнеров, сертификат перенесён со старого,
HTTPS отвечает 200 через `--resolve`. Бот на старом сервере остановлен, на новом запущен.
...
**Этап 15 — переезд на `89.125.121.199`** (в работе с 2026-09-10).
Порядок: подготовка сервера (swap 2 ГБ, Docker) → клон `master` + `.env` → перенос тома
`app-data` со старого сервера → старт стека и проверка по IP → **только после этого**
переключение DNS на Beget и остановка бота на старом сервере. Старый сервер держим
холодным откатом 3–7 дней. Критерии приёмки — в [`07_ROADMAP.md`](07_ROADMAP.md).

**Ветка**: `dev` (прод-ветка `master`, автодеплой пушем в `master`, каталог `/opt/tolkay_voda`).
**Прод**: https://tolkay-voda.ru — сервер 159.194.201.64 (2 ядра / 2 ГБ), docker-compose
