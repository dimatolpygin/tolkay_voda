...
- **CDN Beget отдаёт 523 (Origin unreachable) — на стороне Beget.** С 2026-06-22
  CDN `ceshocofureled.begetcdn.cloud` периодически не может забрать файл из своего
  S3-origin → Liquidsoap не качал треки → эфир играл без звука (тишина). Origin S3
  напрямую стабилен (200). Тикет в поддержку Beget открыт.
  **Временный обход в проде:** эфир играет из локального кэша mp3 на диске сервера
  (том `audio-cache`, `src/lib/audio-cache.js`), не завися от CDN. Детали и план
  отката — в корневом `CLAUDE.md` → «ВРЕМЕННОЕ РЕШЕНИЕ — локальный кэш эфира».
