...
**Этап 10 — Редактирование контента через бота** 🚧. Расширяем `src/bot/` и `src/bot/store.js`:
вход «Редактировать» → список последних прогнозов/статей → правка полей (заголовок/текст/фото)
и удаление. Верификация: клиент в боте правит реальную запись, проверяем изменение на сайте.
