...
**Последнее обновление**: 2026-09-10 (этап 16 взят в работу)
**Текущий этап**: **16 — баннеры под плеером (витрина)**. Майлстоуны 1–3 закрыты и в проде, этап 15 (переезд) закрыт.
**Следующий шаг**: доделать витрину баннеров, затем сразу 17 (веб-панель).
...
**Этап 16 — баннеры под плеером (витрина).** Взят в работу 2026-09-10. Делается
только показ: таблица `ads` (два фиксированных слота), `GET /api/ads`, блок на главной
между «Сейчас играет» и «Прогнозом дня». Интерфейса управления на этом этапе нет —
слоты заполняются напрямую в базе, панель появится на этапе 17. Отдельной ценности
клиенту этап не даёт, показывать имеет смысл уже вместе с 17.
