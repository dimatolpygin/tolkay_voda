...
**Последнее обновление**: 2026-06-16 (этап 4 реализован, ждёт приёмки клиента)
**Текущий этап**: Этап 4 «Блог» — реализован, на проверке у клиента (тег не ставим до приёмки)
**Следующий шаг**: приёмка клиентом блога → tag stage-4-done; затем Этап 5 (Telegram-бот)
...
| 4 | Блог | 🚧 | `stage-4-done` | — | реализован, на приёмке |
...
**Этап**: 4 «Блог» — реализован, ждёт приёмки клиента.
**Ветка**: `dev`
**Что сделано**: сетка статей (`public/js/blog.js`) + пагинация, отдельная страница статьи (`public/post.html` + `public/js/post.js`), CSS страницы статьи. API `/posts?page=` и `/posts/:slug` (с этапа 0). Демо: `npm run seed:posts` (11 статей, 2 страницы). Проверено в браузере (Playwright): сетка 3×3, переход на статью, 404, пагинация — консоль чистая.
