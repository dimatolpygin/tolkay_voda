...
**Последнее обновление**: 2026-06-22 (взят в работу этап 13 — добавление песен через бота)
**Текущий этап**: 🚧 Этап 13 — Добавление песен через бота (майлстоун 3 — управление фонотекой).
**Следующий шаг**: перенос треков в БД → бот «Добавить песню» → перегенерация плейлиста в общий том.
...
🚧 **Этап 13 — Добавление песен через бота** (майлстоун 3). Переносим список треков
из файла `tracks.json` в БД (постоянный том), плейлист эфира генерируем в общий том
app↔liquidsoap (Liquidsoap подхватывает по `reload_mode="watch"` без рестарта), в боте
добавляем «Добавить песню» (mp3 → S3 → БД → перегенерация плейлиста).
**Верификация**: отправить боту mp3 → песня видна в `/api/tracks`, попадает в эфир и
показывает русское название в «сейчас играет» — без передеплоя.
