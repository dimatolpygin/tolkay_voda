...
**Последнее обновление**: 2026-09-11 (панель в проде, ждём приёмку клиентом)
**Текущий этап**: 17 — веб-панель управления `/admin`. Этапы 0–16 закрыты.
**Следующий шаг**: приёмка клиентом с телефона (вход, публикация, баннер). После неё — вывод бота из проекта.
Баннеры до неё ставятся командой (`npm run ads`), клиент сам поставит свой на приёмке этапа 17.
...
телефон, и вывод бота из проекта после приёмки.
...
| 16 | Баннеры под плеером (витрина) | ✅ | `stage-16-done` | 3180a57 | 2026-09-11 |
...
**Этап 17 — веб-панель управления `/admin`.** В проде с 2026-09-11
(`master` = `d2d468e`, Actions №25). Разделы: прогноз дня с выжимкой ИИ и предпросмотром, статьи, песни эфира,
баннеры, смена пароля. Вход по логину и паролю, пароль — scrypt-хеш, сессия 30 дней
в БД, защита от перебора, CSRF, `noindex` и `Disallow`. Вёрстка сначала под телефон,
страницы работают без JavaScript.

Проверено локально на стенде с настоящим S3 и ключом OpenAI: 17 автотестов, выжимка
прогноза, загрузка mp3 и картинки баннера, удаление объектов из S3, вёрстка на 390 и
1280 px. Подробности — в журнале этапа 17 в [`07_ROADMAP.md`](07_ROADMAP.md).

Аккаунт: логин `tolkay`, временный пароль — в `доступы.txt` **вне репозитория**
(репо публичное, паролям там не место). Заведён из `ADMIN_LOGIN`/`ADMIN_PASSWORD`
в `/opt/tolkay_voda/.env`; дальше `.env` не читается, смена пароля в панели деплоем
не откатывается. На первом входе панель требует сменить пароль. Сбросить доступ:
удалить строку из таблицы `admins` и перезапустить `app`.

Баннер клиент уже ставит сам. По его требованию панель **не принимает** картинку
не 16:9: красное сообщение с причиной вместо «сохранено» (см. журнал этапа 17).

**Что осталось:**

1. Приёмка клиентом с телефона: вход, публикация материала, установка баннера.
2. **Только после приёмки** — вывод бота: сервис `bot` из compose, `src/bot/`,
   `BOT_TOKEN`/`BOT_ADMIN_IDS`, зависимость `grammy`, и `docker-compose.override.yml`
   с привязкой Telegram на сервере.

Бот пока работает и остаётся запасным способом публикации до приёмки панели.

Этап 16 закрыт 2026-09-11, витрина баннеров в проде
(Actions №22, коммит `3180a57`). **Слоты пока пусты — на сайте баннеров не видно**,
и это штатно: картинки и ссылки даёт клиент, ставить их он будет сам из панели
этапа 17. До неё слот ставится командой на сервере:
`docker exec tolkay-app node scripts/set-ad.js set 1 <url-картинки> <url-ссылки> ["текст кнопки"]`.

Этап 15 закрыт 2026-09-10: прод переехал на `89.125.121.199`,
...
- **Баннеры переименованы в `partners` — их прятал блокировщик рекламы (2026-09-11).**
  Клиент поставил верный баннер (1200x675, 190 КБ), панель отчиталась «сохранён»,
  карточка приходила в HTML главной, CDN отдавал картинку 200 — а на сайте баннера
  не было. В браузере клиента у `.ad` стоял `display: none`, которого нет ни в одном
  нашем правиле CSS; клон того же узла с другим классом рисовался нормально, а в
  чистом браузере без расширений баннер виден всегда. Это косметический фильтр
  блокировщика: он режет `class`/`id` со словом ad/ads. Слоты платные, поэтому
  переименовано всё, что видит фильтр: классы и id (`partners`/`partner`/`partner__img`/
  `partner__cta`), скрипт `public/js/partners.js`, запрос `GET /api/partners`
  (поле ответа `partners`), префикс новых картинок в хранилище `partners/`.
  Внутренние имена (таблица `ads`, `src/lib/ads-store.js`) не трогали — фильтр их
  не видит. Регрессия закрыта тестом `test/partners.test.js`.
  **Старый баннер лежит по ключу `ads/...`** — новый префикс работает только для
  новых загрузок; перезалить или перенести ключ.

- **Видео в статьях (12.09.2026).** Клиент вставлял в тело статьи код с RuTube,
  и на странице выводилась простыня html вместо плеера. Теперь отдельная строка
  с кодом вставки (или просто со ссылкой на ролик) превращается в плеер 16:9,
  а текст вокруг остаётся на месте — видео его дополняет, а не заменяет.
  Поддержаны RuTube, VK Видео и YouTube. Чужой html в страницу не попадает:
  из вставленного кода берём только адрес ролика и собираем iframe сами, всё
  неопознанное остаётся текстом. Правила разбора лежат в `public/js/video-embed.js`
  и читаются с обоих концов — страницей статьи и сервером (`src/lib/post-text.js`),
  чтобы в анонс ленты и в описание для поисковиков не попал код вставки.

## История закрытий

**2026-09-11 — этап 16 (баннеры под плеером).** Два фиксированных слота в таблице
`ads`, `GET /api/ads`, блок на главной между «Сейчас играет» и «Прогнозом дня»;
на мобиле карточки встают в столбик, пустой слот не рисуется, оба пустых — блока нет.
Текст кнопки сделан необязательным — этим закрыт висевший с 2026-09-04 открытый
вопрос: клиент писал «кнопки не будет», а на его же макете у обеих карточек кнопка
есть. **Главная переехала со статики на свой маршрут** (`src/routes/home.js`):
блок, дорисованный на клиенте по ответу `/api/ads`, толкал вниз прогноз и блог —
замеренный CLS на десктопе 0.091 против 0.0001 при серверной отрисовке. Побочно
баннеры стали видны без JavaScript. Ссылки принимаются только по схемам `http`/`https`,
и негодная строка не только отбивается при записи, но и отсеивается при чтении —
до появления панели база правится руками. На проде проверено на боевом образе
(`fastify.inject`, временная БД в `/tmp`): оба слота рендерятся в HTML с
`rel="noopener noreferrer nofollow sponsored"`, `javascript:`-ссылка отбита, пустые
слоты дают скрытый блок. Деплой не прервал эфир — `liquidsoap` и `icecast` не
пересоздавались (аптайм 22 часа против 13 минут у `app`).
