...
**Последнее обновление**: 2026-09-11 (панель собрана и проверена локально, лежит в `dev`)
**Текущий этап**: 17 — веб-панель управления `/admin`. Этапы 0–16 закрыты.
**Следующий шаг**: выкатить панель в прод (нужна явная команда), завести клиенту логин и пароль, приёмка с телефона, после неё вывести бота.
...
**Этап 17 — веб-панель управления `/admin`.** Код готов и лежит в `dev` (коммит `d86fa98`
и далее). Разделы: прогноз дня с выжимкой ИИ и предпросмотром, статьи, песни эфира,
...
1. Выкладка в прод — **только по явной команде** (мёрж `dev` → `master`).
2. Перед выкладкой или сразу после неё добавить в `/opt/tolkay_voda/.env` на сервере
   `ADMIN_LOGIN` и `ADMIN_PASSWORD` (пароль не короче 8 символов). Аккаунт создаётся
   при первом старте, дальше `.env` не читается. Передать клиенту защищённым способом:
   на первом входе панель сама потребует сменить пароль.
3. Приёмка клиентом с телефона: вход, публикация материала, установка баннера.
4. **Только после приёмки** — вывод бота: сервис `bot` из compose, `src/bot/`,
   `BOT_TOKEN`/`BOT_ADMIN_IDS`, зависимость `grammy`, и `docker-compose.override.yml`
   с привязкой Telegram на сервере.

Бот остаётся рабочим до приёмки: пока панели нет в проде, он единственный способ
публиковать контент.
