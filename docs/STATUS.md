...
**Последнее обновление**: 2026-09-10 (этап 16 написан, ждёт прода)
**Текущий этап**: **16 — баннеры под плеером (витрина)**. Майлстоуны 1–3 закрыты и в проде, этап 15 (переезд) закрыт.
**Следующий шаг**: этап 17 (веб-панель) — витрина без панели клиенту ничего не даёт,
в прод оба этапа едут вместе.
...
**Этап 16 — баннеры под плеером (витрина).** Код написан и проверен локально
2026-09-10, лежит в `dev` (`44f2e62`, `6f8b593`). Таблица `ads` (два фиксированных
слота), `GET /api/ads`, блок на главной между «Сейчас играет» и «Прогнозом дня»,
`npm run ads` для заполнения слотов до появления панели. Главная переехала со статики
на свой маршрут (`src/routes/home.js`): блок отдаётся уже в разметке, иначе он
появлялся после загрузки и толкал страницу — CLS на десктопе 0.091 против 0.0001.

**В прод не выкладывалось, тег не поставлен.** Интерфейса управления на этом этапе
нет, показывать клиенту нечего — по плану этап 16 едет в прод вместе с 17.
Девять локальных критериев приёмки закрыты, остался прод-прогон.
