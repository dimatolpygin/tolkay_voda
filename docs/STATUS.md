...
**Последнее обновление**: 2026-06-22 (взят в работу этап 14 — удаление песен через бота)
**Текущий этап**: 🚧 Этап 14 — Удаление песен через бота (майлстоун 3, завершающий).
**Следующий шаг**: бот «Удалить песню» (список+пагинация → подтверждение → удаление БД+S3 → перегенерация плейлиста).
...
| 14 | Удаление песен (бот) | 🚧 | — | — | — |

Детальные критерии приёмки каждого этапа — в [`07_ROADMAP.md`](07_ROADMAP.md).

## Активная работа

🚧 **Этап 14 — Удаление песен через бота** (майлстоун 3, завершающий). В боте новый
вход «Удалить песню» → список треков с пагинацией → выбор → подтверждение → удаление
записи из БД и объекта из S3 → перегенерация плейлиста (`reload_mode="watch"` → из
эфира без рестарта). Защита: нельзя удалить последний трек. Фундамент (фонотека в БД,
общий том, `deleteTrack`) готов с этапа 13.
**Верификация**: удалить через бота тестовый трек («Человечище») → пропадает из
`/api/tracks` и из эфира; последний трек удалить нельзя.
