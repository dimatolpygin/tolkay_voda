...
**Последнее обновление**: 2026-06-16 (этап 3 реализован, ждёт приёмки клиента)
**Текущий этап**: Этап 3 «Прогноз дня» — реализован, на проверке у клиента (тег не ставим до приёмки)
**Следующий шаг**: приёмка клиентом блока «Прогноз дня» → tag stage-3-done; затем Этап 4 (Блог)
...
| 3 | Прогноз дня | 🚧 | `stage-3-done` | — | реализован, на приёмке |
| 4 | Блог | ☐ | `stage-4-done` | — | — |
...
**Этап**: 3 «Прогноз дня» — реализован, ждёт приёмки клиента.
**Ветка**: `dev`
**Что сделано**: блок «Прогноз дня» на главной (`public/js/forecast.js`), модалка «Читать полный прогноз», API `/forecast/today` + `/forecast/:id` (с этапа 0), демо-запись через `npm run seed:forecast`. Проверено локально в браузере (Playwright): блок и модалка работают, консоль чистая.
